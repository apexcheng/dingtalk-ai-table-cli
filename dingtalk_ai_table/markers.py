...
# 不会被当成 “update 截断、靠 cursor 推进” 的路径误吞。
...
def query_with_marker(
    ...
    while True:
        ...
        try:
            ...
            verified_record_ids = {_extract_record_id(rec) for rec in updated_verify}
            missing_records = [
                record for record in update_payload
                if record['recordId'] not in verified_record_ids
            ]
            ...
            for record in missing_records:
                ...
                try:
                    ...
                    if not updated_one:
                        raise RuntimeError('single record query returned empty result')
